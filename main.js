    ...
    if (line.startsWith("### ")) {
      ...
      while (j < lines.length && !lines[j].trim().startsWith("### ")) {
        ...
        if (trimmedLine.match(/^#[\w\u4e00-\u9fff\/\-_]+(\s+#[\w\u4e00-\u9fff\/\-_]+)*$/)) {
          ...
          if (tagMatches) {
            tags = tagMatches.map((t2) => t2.substring(1));
            hasTags = true;
          }
        } else {
...
var JotView = class extends import_obsidian3.ItemView {
  ...
  highlightMarkdownContent(container, keywords) {
    if (!keywords.length)
      return;
    const walker = document.createTreeWalker(
      container,
  ...
  filterJots() {
    ...
    if (this.selectedTags.size > 0) {
      filtered = filtered.filter(
        (jot) => jot.tags.some((tag) => this.selectedTags.has(tag))
      );
    }
    return filtered;
  }
...
var JotPlugin = class extends import_obsidian5.Plugin {
  ...
  async saveJot(content, tags, source, attachments) {
    ...
    const tagLine = allTags.length > 0 ? allTags.map((t2) => `#${t2}`).join(" ") : "";
    let finalContent = content;
    let attachmentLines = "";
    if (attachments && attachments.length > 0) {
      attachmentLines = attachments.map(
        (att) => att.type === "image" ? `![[${att.path}]]` : `[[${att.path}]]`
      ).join("\n");
    }
    if (tagLine)
      finalContent += `
...
`;
  ...
  async saveToSingleFile(fullDateTime, content) {
    const folder = (0, import_obsidian5.normalizePath)(this.settings.saveFolder);
